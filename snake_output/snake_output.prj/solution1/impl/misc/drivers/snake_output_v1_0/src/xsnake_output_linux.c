...
typedef struct {
    ...
    XSnake_output_uio_map maps[ MAX_UIO_MAPS ];
} XSnake_output_uio_info;

/***************** Variable Definitions **************************************/
static XSnake_output_uio_info uio_info;
...
static int uio_info_read_name(XSnake_output_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSnake_output_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSnake_output_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSnake_output_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSnake_output_Initialize(XSnake_output *InstancePtr, const char* InstanceName) {
	XSnake_output_uio_info *InfoPtr = &uio_info;
